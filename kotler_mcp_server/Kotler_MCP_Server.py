...
from .func_Map_Analyzer import analyze_perceptual_map
from .func_External_Context import get_external_context
...
@app.route('/map_analyzer', methods=['POST'])
def api_map_analyzer():
    """API endpoint for Perceptual Map analysis."""
    if not request.json or 'image_description' not in request.json:
        return jsonify({"error": "Missing 'image_description' in request body"}), 400

    result = analyze_perceptual_map(request.json['image_description'])
    if "error" in result:
        return jsonify(result), 500
    return jsonify(result)

@app.route('/external_context', methods=['POST'])
def api_external_context():
    """API endpoint for simulated web search."""
    if not request.json or 'search_query' not in request.json:
        return jsonify({"error": "Missing 'search_query' in request body"}), 400

    result = get_external_context(request.json['search_query'])
    if "error" in result:
        return jsonify(result), 500
    return jsonify(result)
